---
title: "WB/PCR Knockdown efficiency extraction"
author: 
...
prettier_DT <- function(x, order = NULL, dir = "desc"){
  if(Rmd_output == "html"){
      if(is.null(order)){
      DT::datatable(x, options = list(scrollX = TRUE, autoWidth = FALSE, pageLength = 10), class = "display cell-border compact nowrap")
    }else{
      DT::datatable(x, options = list(scrollX = TRUE, autoWidth = FALSE, pageLength = 10, order = list(order, dir)), class = "display cell-border compact nowrap")
    }
  }
  
  if(Rmd_output == "md"){
    prettier_kabble_extra(x)
  }
}
...
# 
# metadata_WB_kEff_path <- "../Metadata_results/metadata_WB_kEff.tsv"
# metadata_WB_kEff <- readr::read_delim(metadata_WB_kEff_path, show_col_types = F) %>% as_tibble()
# 
# metadata_PCR_kEff_path <- "../Metadata_results/metadata_PCR_kEff.tsv"
# metadata_PCR_kEff <- readr::read_delim(metadata_PCR_kEff_path, show_col_types = F) %>% as_tibble()
# 
...
# 
# metadata_kEff <- metadata_WB_kEff %>%
...
In both situations, they measure the depletion level in the shRNA treated sample against a control non-target shRNA sample. The depletion is normalized using the gene GAPDH as a control. The technique we are most interested in is the Western blotting, a powerful and important procedure to analyze the detection of proteins, particularly in low abundance [@KURIEN2006283]. As such, it represents the actual concentration of protein in the sample and not the RNA abundance.
