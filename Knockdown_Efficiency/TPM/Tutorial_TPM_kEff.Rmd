---
title: "TPM Knockdown efficiency estimation"
author: 
...
<!-- - We need to remove the [versioning number](http://www.ensembl.org/Help/Faq?id=488) from the ENSEMBL id of each gene.  -->
...
metadata_TPM %>%
  ...
  row_spec(5:6, color = "black", background = "#ffaca1") %>%
  row_spec(7:8, color = "black", background = "#c2ffa1")
```
...
metadata_TPM %>%
   ...
   pivot_wider(c(target_gene, cell_line), names_from = "experiment_type", values_from = avg_TPM) %>%
   mutate(Diff = (1-case/control)*100) %>%
  mutate(Diff = paste0(round(Diff, 1), "%")) %>%
...
kEff_cell_line %>%
  ...
  pivot_wider(target_gene, names_from = cell_line, values_from = Diff) %>%
  mutate(Diff = abs(HepG2 - K562)) %>%
...
metadata_TPM %>%
  ...
  dplyr::select(target_gene, cell_line, experiment_type, TPM, gene_quantification_id) %>%
  arrange(cell_line, experiment_type) %>%
  `colnames<-`(c("Target gene", "Cell line", "Cluster", "TPM", "Gene quantification ID")) %>%
  kbl() %>%
  kable_classic(full_width = F, html_font = "Cambria", font_size = 13) %>%
  row_spec(1:2, color = "black", background = "#ffaca1") %>%
  row_spec(3:4, color = "black", background = "#c2ffa1")
```